...
class nilai_siswa extends CI_Controller{
    ...
    function index(){
        $siswa    = $this->db->get_where('tbl_history_kelas',array('nim'=>  $this->session->userdata('nim')))->row_array();
        $kelas         =   "SELECT rb.nama_kelas,rb.kelas,jr.nama_jurusan, mp.nama_mapel
                            ...
                            and j.id_kelas='".$siswa['nim']."'";
        $siswa          =   "SELECT s.nim,s.nama
                            FROM tbl_history_kelas as hk left join tbl_siswa as s on (s.nim=hk.nim) 
                            WHERE hk.nim='".$siswa['nim']."' and hk.id_kelas=".$siswa['id_kelas']." 
   ...
   function nilai_semester(){
    ...
    //     $sql   =  "SELECT sum(uts)/count(nim) as nilai_rata_rata FROM tbl_nilai WHERE id_jadwal=$id_jadwal";
    //     $nilai = $this->db->query($sql)->row_array();
    //     return $nilai['nilai_rata_rata'];
    // }

    // function nilai ($id_jadwal) {
    //     $sql = "SELECT ulangan, uts, uas, (ulangan+uts+uas)/3 as nilai_rata_rata FROM tbl_nilai WHERE id_jadwal=$id_jadwal";
    //     $nilai = $this->db->query($sql)->row_array();
    //     return $nilai;
    // }
